...
void ItemRenderer::draw(Gui& gui, f32 x, f32 y, const ItemStackView& stack) const {
    ...
    if (mesh->flat) {
        ...
        // be added to the same builder the terrain's were.
        for (const render::SpriteUv& uv : mesh->layers) {
            gui.quad(atlas_, x, y, render::kItemCellSize, render::kItemCellSize, uv.u0, uv.v0,
                     uv.u1, uv.v1, 0xFFFFFFFFU);
        }
        return;
    }

    for (const render::ItemQuad& quad : mesh->quads) {
        const u32 base   = quad.tint_index >= 0 ? foliage_tint_ : 0xFFFFFFU;
...
void ItemRenderer::draw_count(Gui& gui, f32 x, f32 y, const ItemStackView& stack) const {
    ...
    // Vanilla's placement: right-aligned two pixels in from the cell's right
    // edge, and nine pixels down from its top.
