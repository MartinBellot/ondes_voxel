...
TEST_CASE("standing inside a block hits it immediately", "[math][raycast]") {
    const auto hit =
        raycast_voxels(Vec3d{0.5, 0.5, 0.5}, Vec3d{1, 0, 0}, 5.0, only(BlockPos{0, 0, 0}));
...
TEST_CASE("an axis-aligned ray reports the entry face", "[math][raycast]") {
    ...
    const auto east =
        raycast_voxels(Vec3d{0.5, 0.5, 0.5}, Vec3d{1, 0, 0}, 10.0, only(BlockPos{3, 0, 0}));
    ...
    const auto down =
        raycast_voxels(Vec3d{0.5, 5.5, 0.5}, Vec3d{0, -1, 0}, 10.0, only(BlockPos{0, 0, 0}));
    REQUIRE(down.has_value());
    REQUIRE(down->face == Direction::Up);  // landed on its top

    const auto north =
        raycast_voxels(Vec3d{0.5, 0.5, 5.5}, Vec3d{0, 0, -1}, 10.0, only(BlockPos{0, 0, 0}));
...
TEST_CASE("reach is respected", "[math][raycast]") {
    ...
    const auto within =
        raycast_voxels(Vec3d{0.5, 0.5, 0.5}, Vec3d{1, 0, 0}, 4.5, only(BlockPos{4, 0, 0}));
    REQUIRE(within.has_value());

    const auto beyond =
        raycast_voxels(Vec3d{0.5, 0.5, 0.5}, Vec3d{1, 0, 0}, 4.5, only(BlockPos{6, 0, 0}));
...
TEST_CASE("a diagonal ray visits every cell it crosses", "[math][raycast]") {
    ...
    (void)raycast_voxels(Vec3d{0.5, 0.5, 0.5}, Vec3d{1, 0, 0.35}, 12.0, [&](BlockPos p) {
...
TEST_CASE("negative coordinates work", "[math][raycast]") {
    ...
    const auto hit =
        raycast_voxels(Vec3d{-0.5, 0.5, 0.5}, Vec3d{-1, 0, 0}, 10.0, only(BlockPos{-4, 0, 0}));
    REQUIRE(hit.has_value());
