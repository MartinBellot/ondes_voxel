...
TEST_CASE("every tag type survives a byte-exact round trip", "[nbt][binary]") {
    ...
    root.put("string", Tag{std::string{"HELLO WORLD THIS IS A TEST STRING \xC3\x85\xC3\x84\xC3\x96!"}});
...
TEST_CASE("an empty list keeps its declared element type through a round trip",
          "[nbt][binary]") {
...
TEST_CASE("a huge list length is rejected before allocating",
          "[nbt][binary][malformed]") {
    const std::vector<u8> bytes{
        0x0A, 0x00, 0x00,
        0x09, 0x00, 0x01, 'a',
...
TEST_CASE("nesting just under the limit is still accepted",
          "[nbt][binary][malformed]") {
...
TEST_CASE("a list of TAG_End with a non-zero count is rejected",
          "[nbt][binary][malformed]") {
...
TEST_CASE("a malformed string is rejected", "[nbt][binary][malformed]") {
    const std::vector<u8> bytes{
        0x0A, 0x00, 0x00,
        0x08, 0x00, 0x01, 'a',
        0x00, 0x02, 0xC3, 0x28,  // bad continuation byte
        0x00,
    };
    const auto result = read(span_of(bytes));
    REQUIRE_FALSE(result.has_value());
